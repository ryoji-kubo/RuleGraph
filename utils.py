...
def get_matching_edges_fast(edge_index_A, edge_index_B):
    '''
    This is a fast matching algorithm using hash functions. O((n_A + n_B) log(n_A))
    Returns for each indices in edge_index_B, the corresponding indices in edge_index_A
    '''
    index, num_match = edge_match(edge_index_A, edge_index_B)
    assert torch.all(num_match <= 1), "Match Function is not working."
    return index